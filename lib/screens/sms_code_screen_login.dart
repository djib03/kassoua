...
import 'package:kassoua/models/user.dart'; // Assuming Utilisateur model is still needed for checking existing users
import 'package:kassoua/constants/colors.dart';
import 'package:kassoua/screens/menu_navigation.dart'; // Navigate to this screen on successful login
...
class _SmsCodeScreenLoginState extends State<SmsCodeScreenLogin>
  ...
  Future<void> _verifyCode() async {
    ...
    try {
      ...
      if (user != null) {
        // Mode connexion : vérifier si l'utilisateur existe dans Firestore
        final userDoc =
        ...
        if (!userDoc.exists) {
          // L'utilisateur n'existe pas dans notre base de données, déconnecter et afficher un message
          await FirebaseAuth.instance.signOut();
          _showErrorSnackBar(
            "Aucun compte trouvé avec ce numéro. Veuillez vous inscrire d'abord.",
          );
          if (mounted) {
            // Optionally navigate back to a registration screen or login screen
            Navigator.of(context).popUntil(
              (route) => route.isFirst,
            ); // Or to a specific login/signup screen
          }
          return;
        }
        // If user exists, continue with successful login flow
      }
    ...
    try {
      await FirebaseAuth.instance.verifyPhoneNumber(
        ...
        verificationCompleted: (PhoneAuthCredential credential) async {
          ...
          _verifyCode(); // Call _verifyCode to handle navigation and user check
        },
